...
final class Codec
{
     ...
     */
    public function encode(iterable $data, array|int $headerOrOffset = [], Format $format = Format::Record): string
    {
        return self::encoder()->encode($this->prepare($data, $headerOrOffset, $format));
    }
     ...
     */
    public function download(iterable $data, ?string $filename = null, array|int $headerOrOffset = [], Format $format = Format::Record): int
    {
        return self::encoder()->download($this->prepare($data, $headerOrOffset, $format), $filename);
    }
     ...
     */
    public function chunk(iterable $data, array|int $headerOrOffset = [], Format $format = Format::Record): Iterator
    {
        return self::encoder()->convert($this->prepare($data, $headerOrOffset, $format));
    }
    ...
    public function write(
        ...
        return self::encoder()->write($this->prepare($data, $headerOrOffset, $format), $to, $context);
    }
    ...
    /**
     * Returns an Iterator representing Tabular data object from your JSON string.
     *
     * @throws NdJsonException
     */
    public function decode(Stringable|string $data, ?int $headerOffset = null): Iterator
    {
        /** @var resource $tmp */
        $tmp = fopen('php://temp', 'r+');
        fwrite($tmp, (string) $data);
        rewind($tmp);

        return $this->read(from: $tmp, headerOffset: $headerOffset);
    }
     ...
     * @throws NdJsonException
     */
    public function read(mixed $from, ?int $headerOffset = null, Format $format = Format::Record, $context = null): Iterator
    {
        $iterator = (new Decoder($this->flags, $this->depth))->read($from, $context);
        [$header, $headerOffsetFound] = self::resolveHeader($iterator, [], $headerOffset);
        if (null !== ($headerOffsetFound ?? $headerOffset) && Format::ArrayWithHeader === $format) {
            ...
            return new MapIterator($iterator, function ($value, $offset) use ($mapper) {
                try {
                    ...
                    throw new DecodingNdJsonFailed(
                        'Unable to map the json line: '.$exception->getMessage(),
        ...
        return new MapIterator($iterator, function (array $record, $offset) use ($header): mixed {
            $assocRecord = [];
            ...
            try {
                ...
                throw new DecodingNdJsonFailed(
                    'Unable to map the json line: '.$exception->getMessage(),
                    data: (string) json_encode($record),
                    offset: $offset,
                    previous: $exception
                );
            }
    ...
    private function prepare(
        ...
        $header = self::header($records, $headerOrOffset, $format);
     ...
     * @return list<string>
     */
    private static function header(iterable $data, array|int $header, Format $format): array
    {
        $headerOffset = null;
        if (is_int($header)) {
            $headerOffset = $header;
