...
final class MapIterator extends IteratorIterator
{
    ...
    /**
     * @param TabularDataProvider|TabularData|iterable<TKey, TValue> $value
     * @param ?callable(TValue, TKey=): TReturn $callable
     *
     * @throws Exception
     * @return Iterator<TReturn|TValue>
     */
    public static function fromIterable(TabularDataProvider|TabularData|iterable $value, ?callable $callable = null): Iterator
    {
        return new self(match (true) {
            $value instanceof TabularDataProvider => $value->getTabularData()->getRecords(),
            $value instanceof TabularData => $value->getRecords(),
