...
`aide-ndjson` is a robust PHP utility for **encoding, decoding, streaming, and tabular parsing of NDJSON (Newlinw-Delimited JSON)**
...
This package provides four global helper functions for working with **NDJSON (Newline Delimited JSON)** data.

- `ndjson_encode()` 

```php
ndjson_encode(iterable $data, int $flags = 0, int $depth = 512): string
```

Encodes a iterable of JSON-serializable values into an NDJSON-formatted string.
...
```php
ndjson_decode(Stringable|string $data, int $flags = 0, int $depth = 512): Iterator
```

Decodes an NDJSON string into a PHP iterable of associative arrays, where each line represents a JSON record.
...
```php
ndjson_write(iterable $data, int $flags = 0, int $depth = 512, $context = null): int
```

Behaves like `ndjson_encode()`, but writes the encoded NDJSON data directly to a file or stream.
...
```php
ndjson_read(mixed $from, int $flags = 0, int $depth = 512, $context = null): Iterator
```

Behaves like `ndjson_decode()`, but reads NDJSON data directly from a file or stream.
...
The global helper functions are simply convenient shortcuts for its core features.
...
The provided callable receives the record must return the transformed version.
...
- `encode()` — returns the encoded NDJSON as a string.
- `chunk()` — yields encoded NDJSON data in chunks using an Iterator.
- `write()` — writes encoded data directly to a file or stream.
- `download()` — encodes and streams NDJSON as a downloadable HTTP response.

#### Encode an array of data to NDJSON string
...
ndjson_write(data: $data, to: __DIR__ . '/users.ndjson')
```
> [!NOTE]
...
During decoding, in addition to JSON options, you can configure how each record is transformed.
...
Transforms each decoded record before it’s returned by the decoder.
...
```php
Codec::decode(Stringable|string $data, ?int $headerOffset = 0): Iterator
```
decodes an NDJSON **string;**


`read()` 

```php
Codec::read(mixed $data, ?int $headerOffset = 0, $context = null): Iterator
```

decodes NDJSON content from a **file** or a **stream**;
...
> [!NOTE]
> ecoding tabular data is **only available** when using the Codec class.

#### Encoding

All encoding methods support **tabular data.**
This is controlled by two elements:

- The output format, via `format(Format $format): self`
- The `$headerOrOffset` parameter available on all encoding methods

> [!WARNING]
> each item of your iterable input MUST be convertable into an array
> otherwise an Exception will be thrown.

Let's use the `encode()` method to illustrate the feature.
...
$json = $codec
    ->asArray()
    ->encode(data: $document, headerOrOffset: 0);

//we have specify the record used for header information
...
$json = $codec
    ->asArrayWithHeader()
    ->encode(data: $document, headerOrOffset: ['Nom', 'Score']);
...
> [!WARNING]
> When using `Format::ArrayWithHeader,` you must provide either:
...
During decoding the header information is split in two parameters `$headerOffset`
to tell the system which record is used as header during decoding and the `$header`
parameter to specify a custom header to replace the data extracted from the
JSON record found at `$headerOffset` if it was specified.

Let's illustrate the behaviour with the following NDJSON:
...
```php
$tabular = (new Codec())->decode(data: $ndjson);
...
```php
$tabular = (new Codec())->decode(data: $ndjson, headerOffset: 0);
...
$tabular = (new Codec())
    ->asArrayWithHeader()
    ->decode(data: $ndjson, headerOffset: 0);
