...
```php
ndjson_read(mixed $from, int $flags = 0, int $depth = 512, $format = Format::Record, $context = null): Iterator
```
...
```php
Codec::decode(Stringable|string $ndjson, Format $format = Format::Record, array|int $headerOrOffset = []): Iterator
```

`read()` — Decodes NDJSON content from a **file** or a **stream**;

```php
Codec::read(mixed $from, Format $format = Format::Record, array|int $headerOrOffset = [], $context = null): Iterator
```
